"""Launch Task 3 sweep. KGE + structural heuristics (CN/RA/L3, sparse so feasible
on the ~3.66M graph); no GNN (message-passing on 3.66M is infeasible, like Task 2
CompGCN). Run:
D:\\Anaconda\\envs\\torch\\python.exe experiments/task3/launch.py  [--smoke]
"""
import argparse
import subprocess
import sys
...
def main():
    ...
    a = ap.parse_args()
    jobs = [("TransE", "B")] if a.smoke else [(m, s) for m in a.models for s in a.settings]
